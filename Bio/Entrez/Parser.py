...
class DataHandler(object):
    ...
    def schemaHandler(self, name, attrs):
        ...
        if not handle:
            ...
            self.save_xsd_file(os.path.basename(schema), schema)
            handle.close()
    ...
    def save_dtd_file(self, filename, text):  #### cail
        import logging
    ...
    def save_xsd_file(self, filename, text):  #### cail
        import logging
        logging.critical(filename)
        logging.info(text)
    ...
    def externalEntityRefHandler(self, context, base, systemId, publicId):
        ...
        if not handle:
            ...
            self.save_dtd_file(filename, url)
            handle = BytesIO(text)
    ...
    def _initialize_directory(self):
        ...
        #### cail
        self.local_dtd_dir = os.path.join(os.path.split(__file__)[0], 'DTDs')
        self.local_xsd_dir = os.path.join(os.path.split(__file__)[0], 'XSDs')
